...
app.get('/api/admin/statistics', authenticateToken, async (req, res) => {
	try {
		...
		const [totalPosts, totalUsers, todayVisitors, totalVisitors, activeChatSessions] =
			await Promise.all([
				prisma.post.count(),
				prisma.user.count(),
...
function generateAIResponse(message) {
	...
	const responses = {
		...
		'liên hệ': {
			message:
				'Bạn có thể liên hệ qua số điện thoại: +84 123 456 789 hoặc email: info@ttphcn-haiduong.vn',
			needsHuman: false
		},
		'phục hồi': {
			message:

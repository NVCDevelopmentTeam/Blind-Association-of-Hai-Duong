<script>
		...
		try {
			...
			if (emailInput && passwordInput) {
				...
				if (emailInput.validity.valid === false) {
					addTestResult('Admin Login - Email Validation', 'PASS', 'Email validation working correctly');
				} else {
				...
				addTestResult('Admin Login - Password Validation', 'PASS', 'Password validation implemented');
			} else {
		...
		try {
			...
			if (shareButtons.length > 0) {
				...
				if (document.activeElement === firstShareButton) {
					addTestResult('Share Features - Keyboard Navigation', 'PASS', 'Share buttons are keyboard accessible');
				} else {
					addTestResult('Share Features - Keyboard Navigation', 'FAIL', 'Share buttons not keyboard accessible');
				}
			} else {
			...
			if (navigator.clipboard) {
				...
				addTestResult('Share Features - Clipboard', 'WARN', 'Clipboard API not available, fallback needed');
			}
		} catch (error) {
	...
	async function testAccessibility() {
		try {
			...
			addTestResult('Accessibility', status, `Score: ${accessibilityScore}/${maxScore} (${percentage}%)`);
		} catch (error) {
	...
	async function testResponsiveDesign() {
		try {
			...
			if (responsiveElements.length > 0) {
				addTestResult('Responsive Design - Classes', 'PASS', `Found ${responsiveElements.length} responsive elements`);
			} else {
				addTestResult('Responsive Design - Classes', 'WARN', 'No responsive classes found');
			}
			
			// Test mobile menu
			const mobileMenuButton = document.querySelector('[aria-label*="mobile menu"], [aria-label*="Toggle"]');
			if (mobileMenuButton) {
...
{#if showTestSuite}
	<div class="fixed bottom-16 left-4 z-50">
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-600 w-96">
					...
					<div class="space-y-2">
						{#each testResults as result}
							<div class="flex items-start gap-2 p-2 bg-gray-50 dark:bg-gray-700 rounded">
								<i class="fas {getStatusIcon(result.status)} {getStatusColor(result.status)} mt-0.5"></i>

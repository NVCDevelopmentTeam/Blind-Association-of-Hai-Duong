...
{#if showControls}
	<div
		class="fixed top-20 right-4 z-50 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 p-6 w-80 max-w-[calc(100vw-2rem)]"
	>
		...
		<div class="mb-4">
			<label class="flex items-center cursor-pointer">
				<input type="checkbox" bind:checked={highContrast} class="sr-only" />
				<div class="relative">
					<div
						class="w-10 h-6 bg-gray-200 dark:bg-gray-700 rounded-full shadow-inner transition-colors {highContrast
							? 'bg-purple-600'
							: ''}"
					></div>
					<div
						class="absolute w-4 h-4 bg-white rounded-full shadow top-1 transition-transform {highContrast
							? 'translate-x-5'
							: 'translate-x-1'}"
					></div>
				</div>
				<span class="ml-3 text-sm text-gray-700 dark:text-gray-300"> Chế độ tương phản cao </span>
			</label>
		</div>
		...
		<div class="mb-4">
			<label class="flex items-center cursor-pointer">
				<input type="checkbox" bind:checked={reducedMotion} class="sr-only" />
				<div class="relative">
					<div
						class="w-10 h-6 bg-gray-200 dark:bg-gray-700 rounded-full shadow-inner transition-colors {reducedMotion
							? 'bg-purple-600'
							: ''}"
					></div>
					<div
						class="absolute w-4 h-4 bg-white rounded-full shadow top-1 transition-transform {reducedMotion
							? 'translate-x-5'
							: 'translate-x-1'}"
					></div>
				</div>

...
<section class="space-y-8">
	{#if loading}
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
			...
			<div
				class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden animate-pulse"
			>
			...
			{#each Array(4) as _}
				<div
					class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 animate-pulse"
				>
	...
	{:else if posts.length === 0}
		<div class="text-center py-12">
			...
			<p class="text-gray-600 dark:text-gray-400">Hiện tại chưa có bài viết nổi bật nào</p>
		</div>
	{:else}
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
			<!-- Main Featured Post -->
			{#if posts[0]}
				<article
					class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden group"
				>
					<div class="relative">
						<img
			...
			{#each posts.slice(1, 5) as post}
				<article
					class="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-200 dark:border-gray-700 p-4 group"
				>
					<div class="flex space-x-4">
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center justify-between mb-2">
								...
								{/if}
								<time datetime={post.publishedAt} class="text-xs text-gray-500 dark:text-gray-400">
									{new Date(post.publishedAt).toLocaleDateString('vi-VN')}
								</time>
							</div>

							<h3
								class="text-sm font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
							>
